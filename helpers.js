...
const helpers = {};
helpers.sendToRabbit = function(data) {
  console.log(data, typeof data);
  if(!data || typeof data !== 'string') return console.log('Error data');
  data = JSON.parse(data);
  if(data.left) data.left = data.left * 1;
  if(data.left) data.right = data.right * 1;
  ...
  nc.addr('127.0.0.1').port(10543).connect().send(data, function(err) {
    ...
    else console.log("netcat data sent");
  }).close(function() {
...
helpers.defineBlocksWithJsonArray([
  {
...
helpers.JavaScript = {};
helpers.JavaScript['blink_nose_fuchsia'] = function(block) {
  var code = 'ledBlink(0, "fuchsia");\n';
  return code;
};
helpers.JavaScript['blink_nose'] = function(block) {
  var color = String(block.getFieldValue('COLOR'));
  var code = 'ledBlink(0, "' + color + '");\n';
  return code;
};
helpers.JavaScript['blink_middle'] = function(block) {
  var color = String(block.getFieldValue('COLOR'));
  var code = 'ledBlink(2, "' + color + '");\n';
  return code;
};
helpers.JavaScript['turn_led_on'] = function(block) {
...
helpers.JavaScript['turn_led_on_simple_colors'] = function(block) {
  var led = String(block.getFieldValue('LED'));
  var color = String(block.getFieldValue('COLOR'));
  var ledIx = ["BOTTOM", "RIGHT", "MIDDLE", "LEFT", "NOSE"].indexOf(led);
  var code = 'led(' + ledIx + ', "' + color + '");\n';
...
helpers.JavaScript['turn_led_off'] = function(block) {
  var led = String(block.getFieldValue('LED'));
  var ledIx = ["BOTTOM", "RIGHT", "MIDDLE", "LEFT", "NOSE"].indexOf(led);
  var code = 'led(' + ledIx + ', "black");\n';
  return code;
};
helpers.JavaScript['move_left_ear_one_step'] = function(block) {
  var code = 'ear(0, 1);\n';
  return code;
};
helpers.JavaScript['move_ear_one_step'] = function(block) {
  var ear = String(block.getFieldValue('EAR'));
  var earIx = ["LEFT", "RIGHT"].indexOf(ear);
  var code = 'ear(' + earIx + ', 1);\n';
  return code;
};
helpers.JavaScript['move_ear'] = function(block) {
  var ear = String(block.getFieldValue('EAR'));
  var earIx = ["LEFT", "RIGHT"].indexOf(ear);
...
helpers.JavaScript['play_note'] = function(block) {
  var sound = String(block.getFieldValue('SOUND'));
  var soundPath = SoundPaths[sound];
  var code = 'playSound("' + soundPath + '");\n';
  return code;
};
helpers.JavaScript['play_notes'] = function(block) {
  var sound = String(block.getFieldValue('SOUND'));
  var soundPath = SoundPaths[sound];
  var code = 'playSound("' + soundPath + '");\n';
  return code;
};
helpers.JavaScript['say_word'] = function(block) {
  var sound = String(block.getFieldValue('SOUND'));
  var soundPath = SoundPaths[sound];
  var code = 'playSound("' + soundPath + '");\n';
  return code;
...
function initInterpreterNabaztag(interpreter, scope) {
  ...
  helpers.JavaScript.addReservedWords('ledBlink');
  helpers.JavaScript.addReservedWords('led');
  helpers.JavaScript.addReservedWords('ear');
  helpers.JavaScript.addReservedWords('playSound');
